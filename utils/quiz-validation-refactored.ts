...
export function correctLatexInJsonContent(jsonContent: string): LaTeXCorrectionResult {
  ...
  latexMatches.forEach((originalQuotedMatch, matchIndex) => {
    let contentInsideQuotes = originalQuotedMatch.slice(1, -1)
    ...
    const commonFixes = [
      ...
      { pattern: /(?<!\\)\\\[/g, replacement: "\\\\[", description: "\\[ → \\\\[ (display math)" },
      { pattern: /(?<!\\)\\\]/g, replacement: "\\\\]", description: "\\] → \\\\] (display math)" },
    ...
    contentInsideQuotes = contentInsideQuotes.replace(environmentPattern, (match, command, env) => {
      matchCorrections++
      correctionDetails.push(`Context ${matchIndex + 1}: Fixed \\${command}{${env}} → \\\\${command}{${env}}`)
      return `\\\\${command}{${env}}`
    })
...
export function validateAndCorrectQuizModule(data: any): {
  ...
  if (typeof data === "string") {
    ...
    try {
      ...
      return {
        validationResult: { isValid: false, errors: ["Failed to stringify input data for LaTeX correction."] },
      }
    }
  }
...
function validateQuestion(question: any, chapterId: string, questionIndex: number): string[] {
  ...
  if (typeof question.explanationText !== "string" || question.explanationText.trim() === "") {
    errors.push(`${prefix}: Missing or invalid 'explanationText' property (must be non-empty string)`)
  }
  ...
  if (!Array.isArray(question.options)) {
    ...
    if (question.type === "true_false") {
      if (
        ...
        !question.options.find((opt: QuizOption) => opt.optionId === "true" && opt.optionText === "True") ||
        !question.options.find((opt: QuizOption) => opt.optionId === "false" && opt.optionText === "False")
      ) {
        errors.push(
      ...
      question.options.forEach((option: any, optionIndex: number) => {
        ...
        if (typeof option.optionId !== "string" || option.optionId.trim() === "") {
          errors.push(`${prefix}, Option ${optionIndex + 1}: Missing or invalid 'optionId' (must be non-empty string)`)
        }
...
export function validateSingleQuestion(data: any): ValidationResult {
  ...
  if (typeof data.questionId !== "string" || !data.questionId.trim()) errors.push("Missing or invalid 'questionId'")
  ...
  if (!Array.isArray(data.options) || data.options.length === 0) errors.push("Missing or invalid 'options' array")
  else {
    if (data.type === "true_false") {
      if (
        data.options.length !== 2 ||
        !data.options.find((opt: QuizOption) => opt.optionId === "true" && opt.optionText === "True") ||
        !data.options.find((opt: QuizOption) => opt.optionId === "false" && opt.optionText === "False")
      ) {
        errors.push(
  ...
  if (!Array.isArray(data.correctOptionIds) || data.correctOptionIds.length === 0)
    ...
    if (data.type === "true_false") {
      if (
        ...
        errors.push(`For 'true_false' type, correctOptionIds must be an array with a single string: 'true' or 'false'.`)
      }
    }
...
export function normalizeQuizModule(data: any): QuizModule {
  const normalizedChapters = data.chapters.map((chapter: any) => {
    ...
    const answeredQuestions = normalizedQuestions.filter((q: QuizQuestion) => q.status !== "not_attempted").length
    const correctAnswers = normalizedQuestions.filter((q: QuizQuestion) => (q.timesAnsweredCorrectly || 0) > 0).length
    return {
      ...chapter,
...
export function recalculateChapterStats(chapter: QuizChapter) {
  ...
  chapter.correctAnswers = chapter.questions.filter((q) => (q.timesAnsweredCorrectly || 0) > 0).length
  ...
  while (lineIndexRef.value < lines.length) {
    ...
    for (const stopper of stopperKeywords) {
      ...
      const isStopperMatch =
        (stopper === "---" && trimmedLine === stopper) || (stopper !== "---" && trimmedLine.startsWith(stopper))
      if (isStopperMatch) {
...
function recoverAndSkipToNextEntry(lines: string[], lineIndexRef: LineIndexRef): void {
  ...
  console.warn(`[Parser Recovery] Attempting to recover from error near line ${initialErrorLine}. Skipping lines...`)
  while (lineIndexRef.value < lines.length) {
    const line = lines[lineIndexRef.value]
    const trimmedLine = line.trim()
  ...
  console.warn(`[Parser Recovery] Reached end of file while recovering from line ${initialErrorLine}.`)
}
...
export function parseMarkdownToQuizModule(markdownContent: string): MarkdownParseResult {
  ...
  if (currentLine && currentLine.startsWith("# ")) {
    ...
    } else if (currentLine && currentLine.startsWith("_") && currentLine.endsWith("_") && currentLine.length > 2) {
    ...
    if (currentLine === "---") {
      ...
      errors.push("[Warning] Expected '---' separator after module header, not found. Parsing continues.")
    }
  } else {
    errors.push("[Error] Markdown must start with a module title (e.g., '# Module Title'). Cannot parse.")
  ...
  while (lineIndexRef.value < lines.length) {
    ...
    while (lineIndexRef.value < lines.length) {
      ...
      try {
        ...
        if (isTrueFalse) {
          ...
          const questionTextContent = parseBlockContent(lines, lineIndexRef, ["**Correct:**", "**Ans:**"])
          questionText = cleanText(rawQuestionHeaderText + (questionTextContent ? "\n" + questionTextContent : ""))
          if (!questionText) {
          ...
          if (currentLine && (currentLine.trim() === "**Options:**" || currentLine.trim() === "**Opt:**")) {
          ...
          if (!currentLine || !(currentLine.startsWith("**Correct:**") || currentLine.startsWith("**Ans:**"))) {
            throw new Error(
          ...
          const correctAnswerText = (
            currentLine.startsWith("**Correct:**") ? currentLine.substring(10) : currentLine.substring(8)
          )
            .trim()
            .toLowerCase()
          ...
          if (!currentLine || currentLine.trim() !== "**Exp:**") {
            throw new Error(`Expected '**Exp:**' for T/F question ${questionId}, found: "${currentLine || "EOF"}"`)
          }
          const explanationTextContent = parseBlockContent(lines, lineIndexRef, explanationStoppers)
          ...
          const questionTextContent = parseBlockContent(lines, lineIndexRef, ["**Options:**", "**Opt:**"])
          questionText = cleanText(rawQuestionHeaderText + (questionTextContent ? "\n" + questionTextContent : ""))
          if (!questionText) {
          ...
          while (lineIndexRef.value < lines.length) {
            ...
            const optionMatch =
              optionLineTrimmed.match(/^\*\*A(\d+):\*\*(.*)$/) || optionLineTrimmed.match(/^-\s*\*\*A(\d+):\*\*(.*)$/)
            if (!optionMatch) {
            ...
            const firstLineOptionText = cleanText(optionMatch[optionMatch[0].startsWith("-") ? 2 : 2])
            ...
            const currentOptionText = cleanText(
              firstLineOptionText + (remainingOptionTextContent ? "\n" + remainingOptionTextContent : ""),
            )
            if (!currentOptionText) {
              errors.push(`[Warning] MCQ Question ID ${questionId}, Option ${optionLabel}: Option text is empty.`)
            }
            options.push({ optionId: parsedOptionId, optionText: currentOptionText })
          ...
          currentLine = consumeLine(lines, lineIndexRef)
          if (!currentLine || currentLine.trim() !== "**Exp:**") {
            throw new Error(`Expected '**Exp:**' for MCQ question ${questionId}, found: "${currentLine || "EOF"}"`)
          }
          const explanationTextContent = parseBlockContent(lines, lineIndexRef, explanationStoppers)
          explanationText = cleanText(explanationTextContent)
          if (!explanationText) {
    ...
    console.log(`Successfully parsed chapter: ${chapterName} (ID: ${chapterId}) with ${questions.length} questions.`)
