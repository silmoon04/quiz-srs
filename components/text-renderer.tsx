  ...
  useEffect(() => {
    const processAndRenderContent = async () => {
      try {
          ...
          (match, language, code) => {
            ...
            if (lang === "array") {
              ...
              if (itemsString.includes(":")) {
                try {
                  const pairs = itemsString
                    .split(",")
                    .map((pairStr) => pairStr.trim());
            ...
            blockIndex++;
            console.log(
              `Extracted fenced code block (${lang}): ${placeholder}`,
            );
            return placeholder;
          },
        );
        ...
        console.log(
          "After Stage 4 - htmlContent sample:",
        ...
        const processedSegments = segments.map((segment, index) => {
          ...
          console.log(
            `Segment ${index} (TEXT): ${textContent.substring(0, 100)}...`,
          );
          ...
          console.log(
            `Processed segment ${index}: ${textContent.substring(0, 100)}...`,
          );
          return textContent;
        });
        ...
        console.log(
          "After Stage 5 - markdownProcessedHtml sample:",
          markdownProcessedHtml.substring(0, 300),
        );
        ...
        for (let i = extractedElements.length - 1; i >= 0; i--) {
          ...
          const escapedPlaceholder = element.placeholder.replace(
            /[.*+?^${}()|[\]\\]/g,
            "\\$&",
          );
          ...
          console.log(
            `Looking for in text: ${finalHtml.includes(element.placeholder) ? "FOUND" : "NOT FOUND"}`,
          );

          if (finalHtml.includes(element.placeholder)) {
            finalHtml = finalHtml.replace(
              new RegExp(escapedPlaceholder, "g"),
              element.processedHtml,
            );
            console.log(`Successfully replaced: ${element.placeholder}`);
          } else {
            console.warn(
              `Placeholder not found in final HTML: ${element.placeholder}`,
            );
          }
        }
